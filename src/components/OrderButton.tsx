...
const OrderButton:React.FC<ButtonType> = ({orderClass, title, leftIcon, orderCount}) => {
  return (
    <button className={`${orderClass} group hover:bg-[#EB5A1E] hover:text-white duration-300 w-[155px] py-[10px] flex justify-center items-center gap-[7px] rounded-[30px] border-[2px] font-semibold text-[16px] text-[#EB5A1E] border-[#EB5A1E]`}>
        {leftIcon}
        <span>{title}</span>
